...
def hard_transfer():
    ...
    saver = tf.compat.v1.train.import_meta_graph(os.path.join(os.path.join(DIR, SD), 'CNN.ckpt.meta'))
    X_ = tf.compat.v1.get_collection("X_")[0]
    h_pool3_reshape = tf.compat.v1.get_collection("h_pool3_reshape")[0]
    keep_prob = tf.compat.v1.get_collection("keep_prob")[0]
    ...
    R_n = Tr_R_X.shape[1]
    R_X_ = tf.compat.v1.placeholder("float", [None, R_n])
    R_Y_ = tf.compat.v1.placeholder("float", [None, CLASS])

    ## FC4: fully connected, dropout
    R_W_fc4 = tf.Variable(tf.random.truncated_normal([R_n, 128], stddev=0.1))
    R_b_fc4 = tf.Variable(tf.constant(0.1, shape=[128]))
    R_h_fc4 = tf.matmul(R_X_, R_W_fc4) + R_b_fc4
    R_keep_prob = tf.compat.v1.placeholder(tf.float32)
    R_h_fc4_drop = tf.nn.dropout(R_h_fc4, rate=1 - (R_keep_prob))

    ## FC5: readout layer
    R_W_fc4 = tf.Variable(tf.random.truncated_normal([128, CLASS], stddev=0.1))
    R_b_fc4 = tf.Variable(tf.constant(0.1, shape=[CLASS]))
    R_y_conv = tf.compat.v1.nn.xw_plus_b(R_h_fc4_drop, R_W_fc4, R_b_fc4)

    ## loss
    R_cross_entropy = tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(labels=tf.stop_gradient(R_Y_), logits=R_y_conv))
    R_train_step = tf.compat.v1.train.AdamOptimizer(1e-4).minimize(R_cross_entropy)
...
def soft_transfer():
    ...
    saver = tf.compat.v1.train.import_meta_graph(os.path.join(os.path.join(DIR, SD), 'CNN.ckpt.meta'))
    X_ = tf.compat.v1.get_collection("X_")[0]
    Y_ = tf.compat.v1.get_collection("Y_")[0]
    y_conv = tf.compat.v1.get_collection("y_conv")[0]
    train_step = tf.compat.v1.get_collection("train_step")[0]
    keep_prob = tf.compat.v1.get_collection("keep_prob")[0]
...
SD = 'AA_ATL_DFW_1500_1759'
TD = 'DL_ATL_DFW_1500_1759'
DIR = '/home/juandiri/Desktop/ExplicabilityOntologyBasedML/SampleData/dataXY'
THREAD_NUM = 2
EPOCH_NUM = 1500
BATCH_SIZE = 100
OPTION = 'S'
